...
function [ResponseRates]=HPCpaper_BehaviorLoop(Events,Animals,Sessions)
...
for ii=1:length(unique(sessions(animals==(min(animals)-1)+i)))
    
%% define the codes for CSstimulus and Lick Response indices, timestamps   
CSstim=[1,2];
Response=3;
SessionIdx=find(animals==(min(animals)-1)+i&sessions==ii);
EventType=Events{1,SessionIdx}{2,1};
Time=Events{1,SessionIdx}{1,1};
