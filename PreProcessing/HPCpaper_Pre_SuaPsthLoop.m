...
%  
%    HPCCells=find(cell2mat(AATC_HPC(i).Area)==1);
%    for ii=1:length(HPCCells)
%    EvokedSig(ii)=ranksum(squeeze(mean(EventMatrix(EventType==1,Pre*30000:(Pre+1.35)*30000,HPCCells(ii)),2)),...
%    squeeze(mean(EventMatrix(EventType==2,Pre*30000:(Pre+1.35)*30000,HPCCells(ii)),2))) ;
% 
%    TraceSig(ii)=ranksum(squeeze(mean(EventMatrix(EventType==1,(Pre+2)*30000:(Pre+3)*30000,HPCCells(ii)),2)),...
%    squeeze(mean(EventMatrix(EventType==2,(Pre+2)*30000:(Pre+3)*30000,HPCCells(ii)),2))) ;
%    end
% % %% Anova  
%         for ii=1:length(HPCCells)
%         Baseline=squeeze(mean(EventMatrix(:,1:Pre*30000,HPCCells(ii)),2));
%         Evoked=squeeze(mean(EventMatrix(:,Pre*30000:Pre*30000+15000,HPCCells(ii)),2));
%         Sustained=squeeze(mean(EventMatrix(:,Pre*30000+15000:Pre*30000+60000,HPCCells(ii)),2));
%         Trace=squeeze(mean(EventMatrix(:,Pre*30000+60000:Pre*30000+90000,HPCCells(ii)),2));
%         Reward=squeeze(mean(EventMatrix(:,Pre*30000+90000:Pre*30000+120000,HPCCells(ii)),2));
...
% %% Firing Rate
% bins=10;
% sets=floor(AATC_HPC(i).RecordingLength/10); 
...
% %     
% TrgDayCounter(CellCounter+1:CellCounter+size(PsthSua,2))=cell2mat(AATC_HPC(i).TrainingDay);
% AnimalCounter(CellCounter+1:CellCounter+size(PsthSua,2))=cell2mat(AATC_HPC(i).Animal);
% SessionCounter(CellCounter+1:CellCounter+size(PsthSua,2))=cell2mat(AATC_HPC(i).Session);
...
% % 
% ExpCounter(CellCounter+1:CellCounter+size(PsthSua,2))=cell2mat(AATC_HPC(i).Exp);
% 
...
% 
% %%Waveform Extraction
% wfsS=cell2mat(AATC_HPC(i).Waveforms.wfs);
% ShankS=cell2mat(AATC_HPC(i).Area);
% includeWFS=cell2mat(AATC_HPC(i).Waveforms.include);
% SpikeV2PS=cell2mat(AATC_HPC(i).Waveforms.SpikeV2P);
% SpikeWidthS=cell2mat(AATC_HPC(i).Waveforms.SpikeWidth);
% DepthS=cell2mat(AATC_HPC(i).Waveforms.Depth);
% 
% %%Waveform Counters
...
for i=1:length(AATC_HPC_PFC)
   ...
   [PsthSua,EventMatrix,EventType]=HPCpaper_Pre_SuaPsth(SpikeTS,ClusterID,GoodClusters,EventTS,EventType,RecordingLength,Pre,Post,Bin);
    HPCCells=find(cell2mat(AATC_HPC_PFC(i).Area)==1);
   for ii=1:length(HPCCells)
...
clearvars -except EvokedSigCounter TraceSigCounter
load AATC_Sua_Psth_1ms
save AATC_Sua_Psth_1ms AATC_Sua_Psth TrgDayCounter AnimalCounter ...
                    CellPerSesCounter SessionCounter...
